...
| Secure AWS access pattern | `charts/render-worker/templates/serviceaccount.yaml`, `charts/render-worker/templates/keda-triggerauthentication.yaml`, `examples/values-security-hardening.yaml`, `docs/security.md` | Run `make test`; inspect placeholder ServiceAccount annotations, KEDA pod identity settings, and secret guidance | Implemented | Least-privilege IAM policies, role bindings, and secret backends remain environment-specific |
...
| NetworkPolicy worker boundary | `charts/render-worker/templates/networkpolicy.yaml`, `examples/values-security-hardening.yaml`, `docs/security.md` | Run `make template-security` or `make test`; inspect worker-only pod selector, empty ingress, DNS egress, and placeholder egress gateway | Implemented | Does not create CNI policy enforcement, VPC endpoints, security groups, NAT gateways, or firewall rules |
| Post-render batch processing | `charts/render-worker/templates/postprocess-job.yaml`, `charts/render-worker/values.yaml`, `scripts/chart-contract-tests.sh` | Render local or AWS examples or run `make test`; inspect Helm hook annotations and the managed-mode opt-out | Implemented | Hook mode avoids normal Helm patching of immutable Job pod templates; live cluster upgrade proof remains environment-specific |
...
| Rendered example validation | `Makefile`, `scripts/render-examples.sh`, `examples/*.yaml` | Run `make render-examples`; inspect default, local, local MinIO, AWS GPU/SQS, and security-hardening manifests | Implemented | Rendering and kubeconform validation are static checks only; no live cluster, AWS SQS, S3, MinIO, IAM, or GPU node behavior is exercised |
...
- Security examples use placeholder IAM role ARNs, placeholder egress labels, and no static AWS keys or renderer license material.
